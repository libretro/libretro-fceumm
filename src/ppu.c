...
static void DoLine(void)
{
	...
	if (scanline >= 240 && scanline != FSettings.totalscanlines)
	{
		X6502_Run(256 + 69);
		scanline++;
		X6502_Run(16);
...
int FCEUPPU_Loop(int skip) {
	...
	if (ppudead) {
		...
		if (FSettings.overclock_enabled && FSettings.vblankscanlines) {
			if (!DMC_7bit || !FSettings.skip_7bit_overclocking) {
				overclocking = 1;
				X6502_Run(FSettings.vblankscanlines * (256 + 85) - 12);
				overclocking = 0;
			}
		}
		PPU_status &= 0x1f;
		...
		if (GameInfo->type == GIT_NSF)
			X6502_Run((256 + 85) * FSettings.normal_scanlines);
		#ifdef FRAMESKIP
		else if (skip) {
			int y;
         ...
         /* manual samples can't play correctly with overclocking */
			if (DMC_7bit && FSettings.skip_7bit_overclocking)
				FSettings.totalscanlines = FSettings.normal_scanlines;
			else
				FSettings.totalscanlines = FSettings.normal_scanlines + (FSettings.overclock_enabled ? FSettings.extrascanlines : 0);

			for (scanline = 0; scanline < FSettings.totalscanlines; ) {	/* scanline is incremented in  DoLine.  Evil. :/ */
				deempcnt[deemp]++;
				/* if ((PPUViewer) && (scanline == PPUViewScanline)) UpdatePPUView(1); */
				DoLine();
				if (scanline < FSettings.normal_scanlines || scanline == FSettings.totalscanlines)
					overclocking = 0;
				else {
					if (DMC_7bit && FSettings.skip_7bit_overclocking) /* 7bit sample started after 240th line */
						break;
					overclocking = 1;
				}
			}
