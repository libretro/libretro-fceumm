...
static void tekprom(void) {
	uint8 prgshift = 0, prgmask = 0x3F, outb_mask = 0x06;
	uint32 bankmode = 0;
	...
	bankmode = ((tkcom[3] & outb_mask) << prgshift);
	switch (tkcom[0] & 7) {
		case 00:
			if (tkcom[0] & 0x80)
				setprg8(0x6000, (((prgb[3] << 2) + 3) & 0x3F) | bankmode);
			setprg32(0x8000, prgmask | bankmode);
			break;
		case 01:
			if (tkcom[0] & 0x80)
				setprg8(0x6000, (((prgb[3] << 1) + 1) & 0x3F) | bankmode);
			setprg16(0x8000, (prgb[1] & prgmask) | bankmode);
			setprg16(0xC000, prgmask | bankmode);
			break;
		case 03: /* bit reversion */
		case 02:
			if (tkcom[0] & 0x80)
				setprg8(0x6000, (prgb[3] & 0x3F) | bankmode);
			setprg8(0x8000, (prgb[0] & prgmask) | bankmode);
			setprg8(0xa000, (prgb[1] & prgmask) | bankmode);
			setprg8(0xc000, (prgb[2] & prgmask) | bankmode);
			setprg8(0xe000, prgmask | bankmode);
			break;
		case 04:
			if (tkcom[0] & 0x80)
				setprg8(0x6000, (((prgb[3] << 2) + 3) & 0x3F) | bankmode);
			setprg32(0x8000, (prgb[3] & prgmask) | bankmode);
			break;
		case 05:
			if (tkcom[0] & 0x80)
				setprg8(0x6000, (((prgb[3] << 1) + 1) & 0x3F) | bankmode);
			setprg16(0x8000, (prgb[1] & prgmask) | bankmode);
			setprg16(0xC000, (prgb[3] & prgmask) | bankmode);
			break;
		...
		case 06:
			...
			setprg8(0xa000, (prgb[1] & prgmask) | bankmode);
			setprg8(0xc000, (prgb[2] & prgmask) | bankmode);
			setprg8(0xe000, (prgb[3] & prgmask) | bankmode);
			break;
	}
}
