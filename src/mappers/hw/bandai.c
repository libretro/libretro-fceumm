...
void (*BANDAI_pwrap)(uint32 A, uint8 V);
void (*BANDAI_cwrap)(uint32 A, uint8 V);
...
static DECLFR(BANDAI_Read) {
	if (eeprom == EEPROM_X24C01) {
		return (X.DB & ~0x010) | (x24c01_read() << 4);
	}
	return (X.DB & ~0x010) | (x24c02_read() << 4);
}
...
DECLFW(BANDAI_Write) {
	...
	case 0x0C:
		...
		break;
	case 0x0D:
		if (eeprom == EEPROM_X24C02) {
			x24c02_write(V);
		} else if (eeprom == EEPROM_X24C01) {
			x24c01_write(V);
		}
		break;
	}
...
void BANDAI_Init(CartInfo *info, EEPROM_TYPE _eeprom_type, int _isFCG) {
	...
	if (eeprom != EEPROM_NONE) {
		...
		if (eeprom == EEPROM_X24C02) {
			...
			AddExState(x24c0x_data + 256, 256, 0, "DATA");
