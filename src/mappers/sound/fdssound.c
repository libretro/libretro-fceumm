...
DECLFR(FDSWaveRead) {
	if (fdso.master_control & WAVE_WRITE_MODE) {
		return (FDS_OPENBUS | fdso.cwave[A & 0x3F]);
	}
	return (FDS_OPENBUS | fdso.cwave[(fdso.cwave_pos >> 21) & 0x3F]);
}
...
DECLFR(FDSEnvVolumeRead) {
	FDSSoundUpdate();
	return (FDS_OPENBUS | fdso.EnvUnits[EVOL].volume);
}

DECLFR(FDSEnvModRead) {
	FDSSoundUpdate();
	return (FDS_OPENBUS | fdso.EnvUnits[EMOD].volume);
}
...
dogk:
	...
	/* Might need to emulate applying the amplitude to the waveform a bit better... */
	{
		...
		return (fdso.cwave[fdso.cwave_pos >> 21] * FSettings.ExpSoundVolume[SND_FDS] * k * 4 / ((fdso.master_control & MASTER_VOLUME) + 2)) >> 8;
	}
}
