...
{
	fk23_regs[5] = V; /* Register 4800 is a separate register, but we use one of the ASIC registers that is otherwise
	                     unused in submapper 5 */
	SyncPRG();
}
static DECLFW(Write5000) {
	if (after_power && A > 0x5010 &&
	    A != 0x5FF3) { /* Ignore writes from $5000-$500F, in particular to $5008, but not $5FF3 */
		after_power = 0;
...
static void M176Reset(void) {
	...
	fk23_regs[1] = fk23_regs[2] = fk23_regs[3] = 0;
...
void Mapper176_Init(CartInfo *info) { /* .NES file */
	...
	if (info->iNES2) {
		...
		if (info->battery) {
			...
			uint32 prg = iNESCart.PRGRomSize / 1024;
			uint32 chr = iNESCart.CHRRomSize / 1024;
