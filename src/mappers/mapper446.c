...
static void M446PW_mmc1(uint32 A, uint8 V) {
    setprg16r(FLASH_CHIP, A, (GetPRGBase() >> 1) | (V & (GetPRGMask() >> 1)));
}
static void M446CW_mmc1(uint32 A, uint8 V) {
    setchr4(A, (GetCHRBase() >> 2) | (V & 0x1F));
}

static void M446PW_mmc3(uint32 A, uint8 V) {
    switch (mapper) {
    case MAPPER_MMC3:
    case MAPPER_TLSROM:
...
static void M446CW_mmc3(uint32 A, uint8 V) {
    switch (mapper) {
    case MAPPER_TLSROM:
...
static void M446PW_vrc24(uint32 A, uint8 V) {
    switch (mapper) {
    case MAPPER_VRC2_22:
    case MAPPER_VRC4_23:
	case MAPPER_VRC4_25:
...
static void M446CW_vrc24(uint32 A, uint32 V) {
    int i;
    switch (mapper) {
    case MAPPER_VRC2_22:
...
static void M446PW_vrc6(uint32 A, uint8 V) {
    setprg8r(FLASH_CHIP, A, GetPRGBase() | (V & GetPRGMask()));
}
static void M446CW_vrc6(uint32 A, uint8 V) {
    setchr1(A, V & 0xFF);
}
...
void Mapper446_Init(CartInfo *info) {
	...
	MMC1_pwrap = M446PW_mmc1;
	MMC1_cwrap = M446CW_mmc1;

	MMC3_Init(info, FALSE, FALSE);
	MMC3_pwrap = M446PW_mmc3;
	MMC3_cwrap = M446CW_mmc3;

	VRC24_Init(info, true, 0x01, 0x02, FALSE, TRUE);
	VRC24_pwrap = M446PW_vrc24;
	VRC24_cwrap = M446CW_vrc24;

	VRC6_Init(info, 0x01, 0x02, FALSE);
	VRC6_pwrap = M446PW_vrc6;
	VRC6_cwrap = M446CW_vrc6;
