...
static uint8 ntaramExtra[0x800]; /* for 4-screen mirroring */
...
static void SyncPRG(void) {
	...
	if (!cfi_mode || !SAVE_FLASH) {
		...
		case 5:
			...
			break;
		case 6:
			setprg32r(REG_B_CHIP, 0x8000, prg_bank_b_mapped >> 2);
			break;
		case 7:
...
static void SyncCHR(void) {
	...
	default:
	case 0:
		setchr8(chr_bank_a >> 3 >> chr_shift);
		break;
	case 1:
		setchr4(0x0000, mapper163_latch >> chr_shift);
		setchr4(0x1000, mapper163_latch >> chr_shift);
		break;
	case 2:
		setchr2(0x0000, chr_bank_a >> 1 >> chr_shift);
		TKSMIR[0] = TKSMIR[1] = chr_bank_a;
		setchr2(0x0800, chr_bank_c >> 1 >> chr_shift);
		TKSMIR[2] = TKSMIR[3] = chr_bank_c;
		setchr1(0x1000, chr_bank_e >> chr_shift);
		TKSMIR[4] = chr_bank_e;
		setchr1(0x1400, chr_bank_f >> chr_shift);
		TKSMIR[5] = chr_bank_f;
		setchr1(0x1800, chr_bank_g >> chr_shift);
		TKSMIR[6] = chr_bank_g;
		setchr1(0x1C00, chr_bank_h >> chr_shift);
		TKSMIR[7] = chr_bank_h;
		break;
	case 3:
		setchr1(0x0000, chr_bank_e >> chr_shift);
		TKSMIR[0] = chr_bank_e;
		setchr1(0x0400, chr_bank_f >> chr_shift);
		TKSMIR[1] = chr_bank_f;
		setchr1(0x0800, chr_bank_g >> chr_shift);
		TKSMIR[2] = chr_bank_g;
		setchr1(0x0C00, chr_bank_h >> chr_shift);
		TKSMIR[3] = chr_bank_h;
		setchr2(0x1000, chr_bank_a >> 1 >> chr_shift);
		TKSMIR[4] = TKSMIR[5] = chr_bank_a;
		setchr2(0x1800, chr_bank_c >> 1 >> chr_shift);
		TKSMIR[6] = TKSMIR[7] = chr_bank_c;
		break;
	case 4:
		setchr4(0x0000, chr_bank_a >> 2 >> chr_shift);
		setchr4(0x1000, chr_bank_e >> 2 >> chr_shift);
		break;
	case 5:
		if (!ppu_latch0) {
			setchr4(0x0000, chr_bank_a >> 2 >> chr_shift);
		} else {
			setchr4(0x0000, chr_bank_b >> 2 >> chr_shift);
		}
		if (!ppu_latch1) {
			setchr4(0x1000, chr_bank_e >> 2 >> chr_shift);
		} else {
			setchr4(0x1000, chr_bank_f >> 2 >> chr_shift);
		}
		break;
	case 6:
		setchr2(0x0000, chr_bank_a >> 1 >> chr_shift);
		...
		break;
	case 7:
		setchr1(0x0000, chr_bank_a >> chr_shift);
		setchr1(0x0400, chr_bank_b >> chr_shift);
		setchr1(0x0800, chr_bank_c >> chr_shift);
		setchr1(0x0C00, chr_bank_d >> chr_shift);
		setchr1(0x1000, chr_bank_e >> chr_shift);
		setchr1(0x1400, chr_bank_f >> chr_shift);
		setchr1(0x1800, chr_bank_g >> chr_shift);
		setchr1(0x1C00, chr_bank_h >> chr_shift);
...
static void SyncMIR(void) {
	if (four_screen) {
		/* setmirror(MI_4); */
		SetupCartMirroring(4, 0, ntaramExtra);
	} else {
...
static DECLFW(M342Write) {
	...
	if (A >= 0x5000 && A < 0x6000 && !lockout) {
		...
		case 3:
			prg_mode = V >> 5;
			chr_bank_a = (chr_bank_a & 0x07) | (V << 3);
			break;
		case 4:
			chr_mode = V >> 5;
		...
		if (lockout) {
			FCEU_printf("mapper code = %d flags: %02x four-screen = %d\n", mapper, flags, four_screen);
		}
	}
	...
	if (A < 0x8000) {
		...
		if (mapper == 6) {
			if (A == 0x5101) {
				...
				if ((A & 0x7000) == 0x5000) {
					...
					case 2:
						prg_mode |= 1;
						prg_bank_a = (prg_bank_a & 0x3F) | ((V & 0x03) << 6);
						mapper163_r0 = V;
						break;
					case 0:
						prg_mode |= 1;
		...
		if (mapper == 12) {
			/* $6000-$7FFF */
			if ((A & 0x6000) == 0x6000) {
				chr_bank_a = (chr_bank_a & 0xE7) | ((V & 0x01) << 4) | ((V & 0x02) << 2);
			}
		}
		...
		*/
		...
		if (mapper == 20) {
			/* $4120-$7FFF */
			if ((flags & 0x02) && (A >= 0x4120)) {
				prg_bank_a = (prg_bank_a & 0xC3) | ((V & 0x0F) << 2) | ((V & 0xF0) >> 2);
			}
		}
		...
		if (mapper == 27) {
			if ((A & 0x6100) == 0x4100) {
				chr_bank_a = (chr_bank_a & 0xC7) | ((V & 0x07) << 3);
				prg_bank_a = (chr_bank_a & 0xF8) | ((V & 0x08) >> 1);
			}
		}
		...
		if (mapper == 28) {
			if ((A & 0x6100) == 0x4100) {
				...
				prg_bank_a = (chr_bank_a & 0xF8) | (V & 0x04);
			}
		}
		...
		if (mapper == 31) {
			if ((A & 0x6000) == 0x6000) {
				chr_bank_a = (chr_bank_a & 0xE3) | ((V & 0x07) << 2);
		...
		if (mapper == 32) {
			if ((A & 0x7000) == 0x7000) {
				prg_bank_a = (prg_bank_a & 0xF7) | ((V & 0x03) << 2);
		...
		if (mapper == 1) {
			if (!(flags & 0x01) || ((A & 0x7000) != 0x1000)) {
				...
				mirroring = 0x02 | ((V >> 4) & 0x01);
			}
		}

		/* Mapper #3 - CNROM */
		if (mapper == 2) {
			chr_bank_a = (chr_bank_a & 0x07) | ((V & 0x1F) << 3);
		}
		...
		if (mapper == 13) {
			...
			case 0x8000:
				...
				case 2:
					...
					break;
				case 3:
					prg_bank_d = (prg_bank_d & 0xC0) | (V & 0x3F);
					break;
				}
				break;
			case 0x9000:
				...
				if (mmc1_load_register & 0x01) {
					...
					case 0xA000:
						...
						if (flags & 0x01) {
							...
							sram_page = 0x02 | (((mmc1_load_register >> 4) & 0x01) ^ 0x01);
						}
					...
					case 0xE000:
						...
						sram_enabled = ((mmc1_load_register >> 5) & 0x01) ^ 0x01;
						break;
					}
		...
		*/
		if (mapper == 24) {
			...
			if (vrc24_compatibility) {
				...
				vrc_2b_addr =
					(((flags & 0x01) ? vrc_2b_low : vrc_2b_hi) << 1) |
					((flags & 0x01) ? vrc_2b_hi : vrc_2b_low);
			} else {
				/* Updated code, not compatible with earlier VRC24 cart variants */
				switch (flags & 0x05) {
				case 0:
					vrc_2b_hi = (((A >> 7) & 0x01) | ((A >> 2) & 0x01)); /* mapper #21 */
			...
			if ((A & 0x7000) == 0x7000) {
				...
				case 2: /* IRQ control */
					...
					if (vrc4_irq_control & 0x02) { /* if E is set */
		...
		if (mapper == 25) {
			...
			if (reg == 1) {
				...
				case 7: /* CHR7 */
					...
					break;
				case 8: /* PRG0 */
					sram_enabled = (V >> 7) & 0x01;
					map_rom_on_6000 = ((V >> 6) & 0x01) ^ 0x01;
		...
		if (mapper == 34) {
			...
			case 0x9000:
				mirroring = V & 1;
				chr_bank_a = (chr_bank_a & 0xBF) | ((V & 0x02) << 5);
				chr_bank_e = (chr_bank_a & 0xBF) | ((V & 0x04) << 4);
				break;
		...
		if (mapper == 35) {
			...
			case 0x8100: /* $81xx */
				mirroring = V & 0x03;
				prg_mode = (prg_mode & 0x03) | ((V >> 2) & 0x04);
			...
			case 0x8300:
				if (!(A & 0x10)) {
					...
					if (!(flags & 0x04)) {
						...
						case 0:
							chr_bank_a = (chr_bank_a & 0x01) | (V << 1);
							break;
						case 1:
							chr_bank_c = (chr_bank_c & 0x01) | (V << 1);
							break;
						case 6:
							chr_bank_e = (chr_bank_e & 0x01) | (V << 1);
							break;
						case 7:
							chr_bank_g = (chr_bank_g & 0x01) | (V << 1);
							break;
						}
					}
				}
		...
		if (mapper == 37) {
			prg_bank_a = (prg_bank_a & 0xF1) | ((V & 0x70) >> 3);
			chr_bank_a = (chr_bank_a & 0x87) | ((V & 0x80) >> 1) | ((V & 0x07) << 3);
			mirroring = 2 | ((V & 0x08) >> 3);
		}
	}
...
static void M342CPUHook(int a) {
	while (a--) {
		...
		if (vrc4_irq_control & 0x02) {
			...
			if ((!(vrc4_irq_prescaler_counter & 0x02) && vrc4_irq_prescaler == 114) ||
				((vrc4_irq_prescaler_counter & 0x02) && vrc4_irq_prescaler == 113)) {
				vrc4_irq_prescaler = 0;
		...
		/* Mapper #73 - VRC3 */
		if (vrc3_irq_control & 0x02) {
			if (vrc3_irq_control & 0x04) { /* 8-bit mode */
		...
		if (mapper42_irq_enabled) {
			...
			if (((mapper42_irq_value >> 13) & 0x03) == 3) {
				X6502_IRQBegin(FCEU_IQEXT);
			} else {
				X6502_IRQEnd(FCEU_IQEXT);
			}
...
void Mapper342_Init(CartInfo *info) {
	...
	ExState(prg_bank_6000_mapped, "P6BM");
	ExState(prg_bank_a_mapped, "PABM");
	ExState(prg_bank_b_mapped, "PBBM");
	ExState(prg_bank_c_mapped, "PCBM");
	ExState(prg_bank_d_mapped, "PDBM");
	...
	AddExState(ntaramExtra, 2048, 0, "ExRM");
}
