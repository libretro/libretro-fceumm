...
static uint32 get_ines_version(iNES_HEADER *header) {
	if ((header->ROM_type2 & 0x0C) == 0x08)
		return 1;
	return 0;
}

static uint32 get_ines_mapper_id(iNES_HEADER *header) {
	...
	uint32 ret;
	switch (header->ROM_type2 & 0x0C) {
	case 0x08:	/* header version is NES 2.0 */
		ret = (((uint32)header->ROM_type3 << 8) & 0xF00) | (header->ROM_type2 & 0xF0) | (header->ROM_type >> 4);
		break;
	case 0x00:	/* header version is iNES */
		ret = (header->ROM_type2 & 0xF0) | (header->ROM_type >> 4);
		break;
	default:	/* any other value is Archaic iNes, byte 7-15 not used */
		ret = (header->ROM_type >> 4);
		break;
	}
	return ret;
}

static void rom_load_ines(iNES_HEADER *header, CartInfo *info, uint32 *prg_size, uint32 *chr_size) {
	*prg_size = header->ROM_size;
	*chr_size = header->VROM_size;
	info->mirror = (header->ROM_type & 8) ? 2 : (header->ROM_type & 1);
	info->battery = (header->ROM_type & 2) ? 1 : 0;
	info->mapper = get_ines_mapper_id(header);
}

static void rom_load_ines2(iNES_HEADER *header, CartInfo *info, uint32 *prg_size, uint32 *chr_size) {
	*prg_size = header->ROM_size;
	*prg_size |= ((header->upper_PRG_CHR_size >> 0) & 0xF) << 8;
	*chr_size = header->VROM_size;
	*chr_size |= ((header->upper_PRG_CHR_size >> 4) & 0xF) << 8;
	info->mirror = (header->ROM_type & 8) ? 2 : (header->ROM_type & 1);
	info->battery = (header->ROM_type & 2) ? 1 : 0;
	info->mapper = get_ines_mapper_id(header);
	info->submapper  = (header->ROM_type3 >> 4) & 0x0F;
	if (header->PRGRAM_size & 0x0F)
		info->PRGRamSize = 64 << ((header->PRGRAM_size >> 0) & 0x0F);
	if (header->PRGRAM_size & 0xF0)
		info->PRGRamSaveSize = 64 << ((header->PRGRAM_size >> 4) & 0x0F);
	if (header->CHRRAM_size & 0x0F)
		info->CHRRamSize = 64 << ((header->CHRRAM_size >> 0) & 0x0F);
	if (header->CHRRAM_size & 0xF0)
		info->CHRRamSaveSize = 64 << ((header->CHRRAM_size >> 4) & 0x0F);
	info->region = header->Region & 3;
}
...
int iNESLoad(const char *name, FCEUFILE *fp) {
	...
	uint32 rom_size_pow2, vrom_size_pow2;
	...
	iNESCart.iNES2 = get_ines_version(&head);

	if (!iNESCart.iNES2)
		rom_load_ines(&head, &iNESCart, &ROM_size, &VROM_size);
	else
		rom_load_ines2(&head, &iNESCart, &ROM_size, &VROM_size);
